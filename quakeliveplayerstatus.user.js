...
function QuakeLiveChatExtender() {
	...
	//quakelive.RegisterModule("friends", q)
	//quakelive.SendModuleMessage("IM_OnMessage", r);
	/*
	// test test
	quakelive.AddHook("IM_OnMessage", function (r) {
		r = quakelive.Eval(r);
		console.log( "r.to: " + r.to );
		console.log( "r.what: " + r.what );
		console.log( "r.from: " + r.who );
	});
    console.log("debug: qlchat loaded");
	*/
}
...
function updateRoster() {
	...
	for(var i in items )  {
		if( items[i].status && typeof items[i].status != "undefined" ) {
			...
			//console.log( "username: " + items[i].jid.username + " status: " + items[i].status );
		}	
	}
}
